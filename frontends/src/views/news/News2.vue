<template>
    <div class="global_navigation">
        <div class="global_navigation_banner"></div>    
    </div>
    <div class="tintuc">
        <div class="container">
                <div id="divListNewsContent" class="content">
                    <div class="row">
                            <div class="col-sm-1"></div>
                            <div class="col-sm-5"> 
                                <div class="row">
                                    <div class="col-sm-6">
                                        <img src="https://media.vietlott.vn//main.vi/11.2022/tkbbt/avatar/6594e55b-f0ee-4b5d-b58a-a1980869e962.jpg" alt="">
                                    </div>

                                    <div class="col-sm-6">
                                        <a target="_self" href="/vi/tin-tuc/tin-trung-thuong/20020-trao-thuong-jackpot-mega-645-tri-gia-723-ty-dong-cho-nguoi-choi-tai-bac-ninh/">
                                            <b style="font-weight:500;color:black;">Trao thưởng Jackpot Mega 6/45 trị giá 72.3 tỷ đồng cho người chơi tại Bắc Ninh</b>
                                        </a>
                                        
                                        <p> Hải Phòng – Ngày 18 tháng 11 năm 2022, ông N.H.H đã nhận giải thưởng Jackpot Mega 6/45 trị giá 72.3 tỷ đồng.</p>
                                        <p class="date"><i class="far fa-calendar-alt"></i> 19/11/2022 14:25</p>
                                    </div>
                                </div>
                            </div>

                            <div class="col-sm-5"> 
                                <div class="row">
                                    <div class="col-sm-6">
                                        <img src="https://media.vietlott.vn//main.vi/11.2022/tkbbt/avatar/6594e55b-f0ee-4b5d-b58a-a1980869e962.jpg" alt="">
                                    </div>

                                    <div class="col-sm-6">
                                        <a target="_self" href="/vi/tin-tuc/tin-trung-thuong/20020-trao-thuong-jackpot-mega-645-tri-gia-723-ty-dong-cho-nguoi-choi-tai-bac-ninh/">
                                            <b style="font-weight:500;color:black;">Trao thưởng Jackpot Mega 6/45 trị giá 72.3 tỷ đồng cho người chơi tại Bắc Ninh</b>
                                        </a>
                                        <p> Hải Phòng – Ngày 18 tháng 11 năm 2022, ông N.H.H đã nhận giải thưởng Jackpot Mega 6/45 trị giá 72.3 tỷ đồng.</p>
                                        <p class="date"><i class="far fa-calendar-alt"></i> 19/11/2022 14:25</p>
                                    </div>
                                </div>
                            </div>
                            <div class="col-sm-1"></div>
                    </div>
                
                    <div class="row">
                            <div class="col-sm-1"></div>
                            <div class="col-sm-5"> 
                                <div class="row">
                                    <div class="col-sm-6">
                                        <img src="https://media.vietlott.vn//main.vi/11.2022/tkbbt/avatar/6594e55b-f0ee-4b5d-b58a-a1980869e962.jpg" alt="">
                                    </div>

                                    <div class="col-sm-6">
                                        <a target="_self" href="/vi/tin-tuc/tin-trung-thuong/20020-trao-thuong-jackpot-mega-645-tri-gia-723-ty-dong-cho-nguoi-choi-tai-bac-ninh/">
                                            <b style="font-weight:500;color:black;">Trao thưởng Jackpot Mega 6/45 trị giá 72.3 tỷ đồng cho người chơi tại Bắc Ninh</b>
                                        </a>
                                        
                                        <p> Hải Phòng – Ngày 18 tháng 11 năm 2022, ông N.H.H đã nhận giải thưởng Jackpot Mega 6/45 trị giá 72.3 tỷ đồng.</p>
                                        <p class="date"><i class="far fa-calendar-alt"></i> 19/11/2022 14:25</p>
                                    </div>
                                </div>
                            </div>

                            <div class="col-sm-5"> 
                                <div class="row">
                                    <div class="col-sm-6">
                                        <img src="https://media.vietlott.vn//main.vi/11.2022/tkbbt/avatar/6594e55b-f0ee-4b5d-b58a-a1980869e962.jpg" alt="">
                                    </div>

                                    <div class="col-sm-6">
                                        <a target="_self" href="/vi/tin-tuc/tin-trung-thuong/20020-trao-thuong-jackpot-mega-645-tri-gia-723-ty-dong-cho-nguoi-choi-tai-bac-ninh/">
                                            <b style="font-weight:500;color:black;">Trao thưởng Jackpot Mega 6/45 trị giá 72.3 tỷ đồng cho người chơi tại Bắc Ninh</b>
                                        </a>
                                        <p> Hải Phòng – Ngày 18 tháng 11 năm 2022, ông N.H.H đã nhận giải thưởng Jackpot Mega 6/45 trị giá 72.3 tỷ đồng.</p>
                                        <p class="date"><i class="far fa-calendar-alt"></i> 19/11/2022 14:25</p>
                                    </div>
                                </div>
                            </div>
                            <div class="col-sm-1"></div>
                    </div>

                    <div class="row">
                            <div class="col-sm-1"></div>
                            <div class="col-sm-5"> 
                                <div class="row">
                                    <div class="col-sm-6">
                                        <img src="https://media.vietlott.vn//main.vi/11.2022/tkbbt/avatar/6594e55b-f0ee-4b5d-b58a-a1980869e962.jpg" alt="">
                                    </div>

                                    <div class="col-sm-6">
                                        <a target="_self" href="/vi/tin-tuc/tin-trung-thuong/20020-trao-thuong-jackpot-mega-645-tri-gia-723-ty-dong-cho-nguoi-choi-tai-bac-ninh/">
                                            <b style="font-weight:500;color:black;">Trao thưởng Jackpot Mega 6/45 trị giá 72.3 tỷ đồng cho người chơi tại Bắc Ninh</b>
                                        </a>
                                        
                                        <p> Hải Phòng – Ngày 18 tháng 11 năm 2022, ông N.H.H đã nhận giải thưởng Jackpot Mega 6/45 trị giá 72.3 tỷ đồng.</p>
                                        <p class="date"><i class="far fa-calendar-alt"></i> 19/11/2022 14:25</p>
                                    </div>
                                </div>
                            </div>

                            <div class="col-sm-5"> 
                                <div class="row">
                                    <div class="col-sm-6">
                                        <img src="https://media.vietlott.vn//main.vi/11.2022/tkbbt/avatar/6594e55b-f0ee-4b5d-b58a-a1980869e962.jpg" alt="">
                                    </div>

                                    <div class="col-sm-6">
                                        <a target="_self" href="/vi/tin-tuc/tin-trung-thuong/20020-trao-thuong-jackpot-mega-645-tri-gia-723-ty-dong-cho-nguoi-choi-tai-bac-ninh/">
                                            <b style="font-weight:500;color:black;">Trao thưởng Jackpot Mega 6/45 trị giá 72.3 tỷ đồng cho người chơi tại Bắc Ninh</b>
                                        </a>
                                        <p> Hải Phòng – Ngày 18 tháng 11 năm 2022, ông N.H.H đã nhận giải thưởng Jackpot Mega 6/45 trị giá 72.3 tỷ đồng.</p>
                                        <p class="date"><i class="far fa-calendar-alt"></i> 19/11/2022 14:25</p>
                                    </div>
                                </div>
                            </div>
                            <div class="col-sm-1"></div>
                    </div>
                    <nav aria-label="Page navigation example">
                        <ul class="pagination justify-content-center">
                          <li class="page-item">
                                <router-link class="page-link" to="/news/news1">«</router-link>
                          </li>
                          <li class="page-item">
                                <router-link class="page-link" to="/news/news2">3</router-link>
                          </li>

                          <li class="page-item">
                                <router-link class="page-link" to="/news/news3">4</router-link>
                          </li>
                          <li class="page-item">
                                <router-link class="page-link" to="/news/news4">5</router-link>
                          </li>
                          <li class="page-item">
                                <router-link class="page-link" to="/news/news5">»</router-link>
                          </li>
                        </ul>
                    </nav>
                </div>
            </div> <!-- /.content -->
        </div>
</template>

<script>
export default {
    name: 'News1',
}
</script>

<style scoped>
.global_navigation .global_navigation_banner{
    background: url('../../assets/images/background/global_navigation_bg.jpg') no-repeat center center;
    background-size: cover;
    min-height: 130px;
}
.container {
    padding-right: 15px;
    padding-left: 15px;
    margin-right: auto;
    margin-left: auto;
    margin-top:80px;
}

.row {
    margin-right: -15px;
    margin-left: -15px;
    margin-bottom: 30px;
}
.box_tintuc {
    color: #001452;
    margin-top: 20px;
}
.tintuc img {
    width: 100%;
    height: 102%;
}
</style>